...
type Candidate = { base: string; anagrams: string[] };
function assertNonEmpty<T>(arr: T[], msg: string): asserts arr is NonEmptyArray<T> {
...
export function pickBaseWord(groups: Map<string, string[]>): Candidate {
  ...
  for (const [, words] of groups) {
    if (words.length >= 2) {
      const base = words[0];
      if (base === undefined) throw new Error("unreachable: words[0] undefined");
...
export function isAnagramOf(base: string, guess: string, dict: Set<string>): boolean {
